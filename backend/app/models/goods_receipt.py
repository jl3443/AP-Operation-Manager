...
class GRNLineItem(TimestampMixin, Base):
    ...
    __table_args__ = (Index("ix_grn_line_items_grn_id", "grn_id"),)

    grn_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("goods_receipts.id", ondelete="CASCADE"), nullable=False
    )
    po_line_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("po_line_items.id"), nullable=False)
