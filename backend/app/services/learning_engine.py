...
from app.models.config import PolicyRule, PolicyRuleStatus, ToleranceConfig
...
def analyze_resolution_patterns(db: Session) -> dict[str, Any]:
    ...
    resolved = (
        db.query(Exception_)
        .filter(Exception_.status == ExceptionStatus.resolved)
        .all()
    )
...
def recommend_threshold_adjustments(db: Session) -> list[dict[str, Any]]:
    ...
    amount_exceptions = (
        db.query(Exception_)
        .filter(Exception_.exception_type == "amount_variance")
        .all()
    )
    ...
    qty_exceptions = (
        db.query(Exception_)
        .filter(Exception_.exception_type == "quantity_variance")
        .all()
    )
...
def suggest_new_rules(db: Session) -> list[dict[str, Any]]:
    ...
    approvals = db.query(AuditLog).filter(AuditLog.action == "status_change").all()
    pending_count = (
        db.query(func.count(Invoice.id))
...
def get_performance_benchmarks(db: Session) -> dict[str, Any]:
    ...
    perfect_matches = (
        ...
        .filter(MatchResult.match_status == MatchStatus.matched)
        .scalar() or 0
    )
    tolerance_matches = (
        db.query(func.count(MatchResult.id))
        .filter(MatchResult.tolerance_applied.is_(True))
        .scalar() or 0
    )
    ...
    benchmarks = {
        "touchless_rate": {
            ...
            "industry_avg": 30.0,
            "best_in_class": 80.0,
            "rating": "above_average" if touchless_rate > 30 else "below_average" if touchless_rate > 15 else "needs_improvement",
        },
        "exception_rate": {
            "current": round(exception_rate, 1),
            "industry_avg": 25.0,
            "best_in_class": 5.0,
            "rating": "best_in_class" if exception_rate < 5 else "above_average" if exception_rate < 25 else "needs_improvement",
        },
...
def get_learning_summary(db: Session) -> dict[str, Any]:
    ...
    return {
        ...
        "maturity_level": (
            "Optimized" if maturity_score >= 75
            else "Managed" if maturity_score >= 50
            else "Developing" if maturity_score >= 25
            else "Initial"
        ),
